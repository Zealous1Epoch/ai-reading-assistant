...
**问题**：点击"下载 PNG"按钮导出思维导图遇到一系列问题——导出空白、文字模糊、节点缺字、留白过大、色彩错误。尝试了至少 10 轮迭代：offscreen SVG getBBox 返回 0、canvas measureText 与 SVG 渲染不一致、CJK 字符宽度计算偏差、markmap color 选项格式错误（数组 vs 函数）。

**解决**：放弃 PNG 导出，改为全屏悬浮弹窗放大查看方案——毛玻璃背景（backdrop-filter: blur）+ 白色圆角卡片 92vw×90vh，markmap 完整铺开，点背景或关闭按钮退出。在面板内显示正常即可，放大直接看无需导出。

---

## 11. AI 生成思维导图带前言废话

**问题**：AI 生成思维导图时，在 markdown 内容前附带一段说明性前言（如"以下是根据您提供的《第三章 谁是真神》内容生成的详尽思维导图..."），导致 markmap 渲染时将前言也当作节点显示。

**解决**：
- 后端 prompt 增加明确约束：「不要输出任何前言、说明、结尾语或客套话。第一行必须是 # 开头的标题」
- 前端新增 `cleanMindmapData()` 函数，渲染前自动定位第一个 `\n# ` 并切除之前的所有废话

---

## 12. AI 工具结果无法收藏

**问题**：收藏夹仅支持聊天 Q&A 的收藏，AI 工具生成的总结、背景调研、读书十问等结果无法保存。

**解决**：
- 收藏数据结构增加 `type` 字段（`'qa'` / `'tool'`），工具收藏附带 `toolId` 和 `toolLabel`
- RightPanel 工具结果标题栏增加 ☆ 收藏按钮（已有结果且非错误时显示）
- 收藏夹列表显示工具类型标签（如"总结""读书十问"）
- 导出时根据 type 区分标题行（`=== AI工具 · 总结 ===` vs `=== 收藏问答 ===`）
- MiddlePanel 聊天收藏同步增加 `type: 'qa'`

---

## 13. 前端白屏（Babel 编译错误）

**问题**：app.jsx 中出现多余 `};` 导致 Babel Standalone 编译失败，整页白屏。

**解决**：定位并删除重复的闭合括号。之后未再出现。

---

## 14. 思维导图层级不够深入

**问题**：初始 prompt 仅要求到 3-4 级标题（书名→章节→主题），每个节点限制 15 字以内，导致思维导图过于简略，缺少论据和细节支撑。

**解决**：
- 重写 prompt，要求六级深度（书名→章节→主题→论点→论据→具体引文/数据/案例）
- 去掉"15字以内"限制，允许完整表达
- 强调五六级是核心，要求引用书中真实内容（人名、事件、数据、原文）
- 上下文从 8000 字扩大到 12000 字
- 单章上下文从 6000 扩大到 10000 字

---

## 15. 章节目录显示缺乏层级感

**问题**：左侧章节导航是扁平列表，所有章节标题等距排列，无法体现章-节层级关系。

**解决**：利用后端 AI 目录识别返回的 TOC 数据（含 level 字段），前端渲染时：
- level=1 章节显示编号序号（1. 2. 3.）
- level=2 小节缩进 pl-8 + 圆点标记
- 无 TOC 数据时退化为章节平铺